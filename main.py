...
def _build_step_cache_matches(blob: bytes) -> list[StepCacheMatch]:
    matches: list[StepCacheMatch] = []
    image_hash = _sha256_bytes(blob)
    for provider in ("gemini", "gpt", "claude"):
        payload = _cache_json_get("cadstep_image", _sha256_text(f"{provider}::{image_hash}"))
        if not payload:
            continue
...
@app.post("/api/assets/delete", response_model=AssetDeleteResponse)
async def delete_asset(payload: AssetDeleteRequest, request: Request) -> AssetDeleteResponse:
    limiter.check(request, "assets-delete")
    success = asset_catalog.delete_asset(payload.asset_rel_path)
    if success:
        message = f"Asset '{payload.asset_rel_path}' deleted successfully."
    else:
        message = f"Asset '{payload.asset_rel_path}' not found or could not be deleted."
    return AssetDeleteResponse(success=success, message=message)
