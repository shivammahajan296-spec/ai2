...
const DEFAULT_STEP_CAD_PROMPT =
  "Role: You are a Senior Python CAD Developer and Mechanical Engineer specializing in the cadquery library cadquery==2.5.2\n\nTask: Analyze the attached image and generate a robust, executable Python script using cadquery to create a parametric 3D model of the object. The output must be a valid STEP file.\n\nCRITICAL SYNTAX RULES (To prevent Errors):\n\nFix pyparsing.exceptions.ParseException:\nNEVER use natural language selectors like faces(\"top\"), faces(\"bottom\"), or faces(\"front\").\nONLY use Axis-based string selectors: \">Z\", \"<Z\", \">X\", \"<X\", \">Y\", \"<Y\".\nExample: Use faces(\">Z\") to select the top face.\nFix AttributeError:\nStrictly adhere to the CadQuery 2.0 Fluent API.\nDo not invent methods. Common valid methods are: .rect(), .circle(), .extrude(), .cutBlind(), .hole(), .fillet(), .chamfer(), .revolve().\nEnsure you are operating on the correct stack (2D vs 3D). You cannot .fillet() a 2D sketch; you must extrude it first.\nImports: Start with import cadquery as cq.\nSTEP 1 — GEOMETRY ANALYSIS\n\nIdentify the primitive shapes (Cylinders, Cuboids, etc.).\nDetect symmetry and features (Holes, Ribs, Chamfers).\nEstimate dimensions (assume mm).\nSTEP 2 — PARAMETRIC SETUP\n\nDefine all dimensions as variables at the top of the script (e.g., length = 100, hole_dia = 5).\nThis allows for easy modification later.\nSTEP 3 — MODELING STRATEGY\n\nStart with a base cq.Workplane(\"XY\") (or appropriate plane).\nUse boolean operations (.union(), .cut()) if the geometry is complex.\nThreads: If threads are visible, model them as a simple cylinder with the major diameter (modeling actual helical threads often breaks kernels; keep it simple for STEP export unless requested otherwise).\nSTEP 4 — CODE STRUCTURE Return only the Python code block. The code must end with the export command.\n\nReference Template (Follow this structure):\n\nimport cadquery as cq\n\n# --- 1. Parameters ---\nheight = 50.0\ndiameter = 25.0\nwall_thickness = 2.0\nfillet_rad = 1.0\n\n# --- 2. Build Geometry ---\n# Base operation\npart = (\n    cq.Workplane(\"XY\")\n    .circle(diameter / 2)\n    .extrude(height)\n)\n\n# Features (Example: Hollow out and fillet top)\npart = (\n    part\n    .faces(\">Z\")  # Select top face using Axis selector ONLY\n    .shell(-wall_thickness) # Shelling\n    .edges(\"|Z\") # Select vertical edges\n    .fillet(fillet_rad)\n)\n\n# --- 3. Export ---\ncq.exporters.export(part, \"generated_model.step\")\nprint(\"Exported generated_model.step\")\n\nSTRICT CADQUERY RULES (MANDATORY)\n• Always create a solid first using .extrude(), .revolve(), or .loft().\n• Never apply fillet or shell before a solid exists.\n• Never use: roundedRect, sortedBy, cadquery.selectors.*, EdgeTypeSelector, Circle, sort_by_distance_to_axis.\n• Only use these selectors: faces(\">Z\"), faces(\"<Z\"), faces(\">X\"), faces(\">Y\"), edges(\"|Z\"), edges(\"|X\"), edges(\"|Y\"), edges(\">Z\"), edges(\"<Z\").\n• Always select edges before calling .fillet().\n• Always select faces before calling .shell().\n• Always reassign operations back to part.\n• Never create a new Workplane after the solid is built.\n• Do not use experimental or undocumented CadQuery APIs.\n• If unsure about a selector, skip the operation.\n• Final object must be stored in variable part.\nBefore applying fillet, compute the minimum edge length and ensure fillet radius <= 30% of the smallest edge; if fillet fails, automatically retry with radius/2 and skip if still failing.\nNever apply fillet to invalid, zero-length, or non-manifold edges; validate the solid before modifying.";
  ...
  items.forEach((item, idx) => {
    ...
    card.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: flex-start; width: 100%;">
        ...
        <button type="button" class="btn tiny secondary delete-asset-btn" title="Delete this asset" style="padding: 4px 8px; font-size: 12px; height: fit-content;">✕</button>
      </div>
    `;
    
    const deleteBtn = card.querySelector(".delete-asset-btn");
    deleteBtn.addEventListener("click", async () => {
      if (confirm(`Delete asset: ${item.asset_rel_path}?\n\nThis cannot be undone.`)) {
        await deleteAsset(item.asset_rel_path);
      }
    });
...
async function deleteAsset(assetRelPath) {
  try {
    setOperationLoading(true, "Deleting asset...");
    const res = await apiPost("/api/assets/delete", { asset_rel_path: assetRelPath });
...
function renderStepCacheLookupResult(res, fileName) {
  ...
  const matchesMarkup = (res.matches || [])
    .map((match) => {
      const promptText = match.prompt ? escapeHtml(match.prompt) : "No prompt saved";
      const stepText = match.step_file
        ? (match.step_file_exists ? `<a href="${match.step_file}" target="_blank" rel="noopener">Open STEP</a>` : "Missing file")
        : "No STEP path saved";
      const codeText = match.code_file
        ? (match.code_file_exists ? `<a href="${match.code_file}" target="_blank" rel="noopener">Open Code</a>` : "Missing file")
        : "No code path saved";
      return `<div class="cache-status-item">
        <strong>${escapeHtml(match.provider)}</strong>
    ...
    .join("");
  el.stepCacheStatus.dataset.state = statusState;
